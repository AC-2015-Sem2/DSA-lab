...
//Repeat this until there are (V-1) edges in the spanning tree.
// Number of edges to be taken is equal to V-1

    while(totalEdges<nrOfVerteces-1)
    {
        ...
        if(predecessor[minEdge.source] != predecessor[minEdge.destination])
        {
            edges[totalEdges] = minEdge;

            if(predecessor[minEdge.source] > predecessor[minEdge.destination])
            {
                min1 = predecessor[minEdge.destination];
                min2= predecessor[minEdge.source];
            }
            else
            {
                min2= predecessor[minEdge.source];
            ...
            for(i=0; i<nrOfVerteces; i++)
            {
                if(predecessor[i] == min2)
                    predecessor[i] = min1;
            }
            totalEdges++;
            printEdges(edges,totalEdges);
            // print the contents of edges[] to display the built of Minimum Spanning Tree
        }
