...
void readMatrixBF(FILE * bf)
{
    ...
    nrOfBFVerteces=n;
    int i,j;
    adjBFMatrix=(int**)malloc(n*sizeof(int*));
    for (i=0; i<n; i++)
    {
        *(adjBFMatrix+i)=(int*)malloc(n*sizeof(int));
    }

    for (i=0; i<n; i++)
        for (j=0; j<n; j++)
            fscanf(bf,"%d",&adjBFMatrix[i][j]);
    fclose(bf);
}
...
void printAdjBFMatrix()
{
    ...
    printf("\nThe Bellman-Ford algorithm matrix is:\n");
    printf("    ");
