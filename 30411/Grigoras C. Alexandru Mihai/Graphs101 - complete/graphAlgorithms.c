...
void kruskal()
{
	...
	while(ct < nrOfVerteces)
	{
		...
		edgeT min=getMinimumEdgeForAdjacencyMatrix(matrix);
		a=min.source;
		b=min.destination;
		c=min.weight;
		min.source=getParent(parent,min.source);
        min.destination=getParent(parent,min.destination);
		//u=find(u);
		//v=find(v);
		if(uni(parent,min.source,min.destination))
		{
			printf("%d edge (%d,%d) =%d\n",ct++,a,b,c);
			mincost +=c;
		}
...
int getnumberofedges(int **matrix)
{
    int i,j,ct=0;
    for (i=0;i<nrOfVerteces;i++)
...
edgeT *getdist(int **matrix)
{
    int i,j,e=0;
    edgeT *edges=(edgeT*)malloc(sizeof(edgeT)*nrOfVerteces);
    for (i=0;i<nrOfVerteces;i++)
        for (j=0;j<nrOfVerteces;j++)
        {
            edges[e].source=i;
...
void bellman_ford(int s) {
    int i, j;
    edgeT *edges=getdist(adjMatrix);
    int e=getnumberofedges(adjMatrix);
    int d[100];
    for (i = 0; i < 100; ++i)
        d[i] = 9999;
    d[s] = 0;
    for (i = 0; i < nrOfVerteces - 1; ++i)
        for (j = 0; j < e; ++j)
            if (d[edges[j].source] + edges[j].weight < d[edges[j].destination])
                 d[edges[j].destination] = d[edges[j].source] + edges[j].weight;
    printDista(d);
}
