...
int nrofDigits(int *a,int n)
{
 ...
 void newarray(int *a, int n, int *endResult )
{
    int i=0;
    for (i=0;i<n;i++){
       a[i]=endResult[i];
...
{
  int digit=10; // possible values to compare (0 to 9)
  int pos=1;
  int g;
  int k=nrofDigits(a,n); //length of the max nr in the array; number of "Sorts"
 for (g=0;g<k;g++)
 {
     int i=0;
     int j;
    ...
    for (i=0;i<digit;i++)
    {
     for (j=0;j<n;j++)
     {
        ...
        {
            comparisons++;
            endResult[h]=a[j];         //putting the numbers in order
            h++;
            assignments+=2;
        }
     }
    }
    newarray(a,n,endResult); //resetting "a" after the partial sort
    pos=pos*10; // going to the next position in the number
assignments++;
    }
}
