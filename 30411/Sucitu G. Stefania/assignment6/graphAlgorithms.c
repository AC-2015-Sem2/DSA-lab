...
void kruskal()
{
    ...
    while(nrOfVisited<=nrOfVerteces-1)
    {
        edgeT edge=getMinimumEdgeForAdjacencyMatrix(adjmat);
        a=u=edge.source;
        b=v=edge.destination;
        u=getParent(parent,u);
        v=getParent(parent,v);
        if (uni(parent,u, v))
        {
            printf("\n added edge is: %c -> %c",a+65,b+65);
            nrOfVisited++;
            mincost+=edge.weight;
        }
...
void printDist(int start,int*d,int **dist)
{
    int i;
    for (i=0; i<nrOfVerteces; i++)
    {
        printf("\n the distance from %c to %c was %d and now it is %d",start+65,i+65,dist[start][i],d[i]);
    }
}
